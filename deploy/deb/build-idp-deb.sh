...
build_root="${repo_root}/build/deb-idp"
...
(
  ...
  test -f "${repo_root}/packages/pod-idp/package.json"
  test -f "${repo_root}/packages/pod-idp/config/css-idp.json"
)

for arch in ${arches}; do
  package_root="${build_root}/package-${arch}"
  runtime_root="${package_root}/opt/oxfordia-pod-idp"
  output_path="${repo_root}/build/oxfordia-pod-idp_${version}_${arch}.deb"
  ...
  install -m 0644 "${repo_root}/deploy/deb/oxfordia-pod-idp.service" "${package_root}/lib/systemd/system/oxfordia-pod-idp.service"
  install -m 0644 "${repo_root}/deploy/deb/oxfordia-pod-idp.default" "${package_root}/etc/default/oxfordia-pod-idp"
  ...
  fpm \
    ...
    --description "Oxfordia Pod IDP - A Community Solid Server identity provider deployment" \
    --license "MIT" \
    --url "https://github.com/OXFORDIA-project/OXFORDIA-node" \
    -d systemd \
    --config-files /etc/default/oxfordia-pod-idp \
    --after-install "${repo_root}/deploy/deb/postinst-idp" \
    --before-remove "${repo_root}/deploy/deb/prerm-idp" \
    -C "${package_root}" \
    .
done
