...
export function KaplanMeierObservationsTester() {
  ...
  const selectedTimeShortcut = resolveGraphPathShortcut(DATA_SCHEMA_NAME, queryDraft.timePath);
  const selectedEventShortcut = resolveGraphPathShortcut(DATA_SCHEMA_NAME, queryDraft.eventPath);
  const selectedGroupByShortcut = queryDraft.groupByPath
    ? resolveGraphPathShortcut(DATA_SCHEMA_NAME, queryDraft.groupByPath)
    : null;
