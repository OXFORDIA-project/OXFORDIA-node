...
if (length(file_arg) == 0) {
  stop(
    "Unable to determine the oxfordiar.data.nemaline script path.",
    call. = FALSE
  )
}
...
temp_lib <- file.path(tempdir(), "oxfordiar-data-nemaline-lib")
...
library(testthat)
library(oxfordiar.data.nemaline, lib.loc = temp_lib)
testthat::test_dir(
