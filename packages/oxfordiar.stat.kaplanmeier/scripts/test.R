...
if (length(file_arg) == 0) {
  stop(
    "Unable to determine the oxfordiar.stat.kaplanmeier script path.",
    call. = FALSE
  )
}
...
temp_lib <- file.path(tempdir(), "oxfordiar-stat-kaplanmeier-lib")
...
library(testthat)
library(oxfordiar.stat.kaplanmeier, lib.loc = temp_lib)
testthat::test_dir(
