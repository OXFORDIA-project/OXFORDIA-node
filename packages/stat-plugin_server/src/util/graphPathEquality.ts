...
import { graphPathContext } from "@oxfordia/stat-plugin_core";
import {
  compareByKey,
  type IriObject,
  getIriValue,
...
function contextIdFor(key: string): string | undefined {
  const contextValue = graphPathContext[key];
...
const COMPACT_TERM_MAP = Object.entries(graphPathContext).reduce<
