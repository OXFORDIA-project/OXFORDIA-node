...
import type { DataPlugin, GraphPathShortcut } from "./types";
...
export function resolveGraphPathShortcut(
  ...
  if (graphPath.name) {
    return (
      getGraphPathShortcutsForDataSchema(dataPlugins, dataSchemaName).find(
  ...
  return (
    getGraphPathShortcutsForDataSchema(dataPlugins, dataSchemaName).find((shortcut) => {
